...
func ReverseSearchDB(h *DbHandler, keyword string, gemini_api_key string) (string, error) {
	query := "{ "
	...
	defer func(client *genai.Client) {
		err := client.Close()
		if err != nil {
			fmt.Println("Unable to close gemini client")
		}
	}(client)

	model := client.GenerativeModel("gemini-1.5-flash")
	prompt := []genai.Part{
		genai.Text("query = " + keyword + " array of strings = " + query + " } . You are required to output the best matching string for given query . If found , just return only that string else return \"NO\" . Just output only the string or NO ; nothing else and output without double inverted commas ; no new lines also at end ; just the string only"),
	}
	resp, err := model.GenerateContent(ctx, prompt...)
	if err != nil {
		return keyword, err
